...
tissues <- c('SI', 'LI', 'THYMUS', 'LN', 'SPLEEN')
cluster_nums <- c(5)
intensity_types <- c('total')
radius <- 500  # Fixed radius value
hr_values <- c(1)
      ...
      for (hr in hr_values) {
        ...
        if (file.exists(quad_file)) {
          ...
          system(paste0(
            ...
            file.path(data_dir, TMC, tissue, paste0('model_all_images_', cluster_num, '_100-', radius, '_', hr, '_', intensity_type, '_across3_self.out')),
            ' -n 1 -c 16 --mem 120G Rscript ./modeling/heterogeneity/train_multi_range_all_images_across3_self.R ',
