...
# 
# get_shuffle_test = function(data){
...
#     # weights = pattern_test$.mpl.W
#     # data = pattern_test[,1:ncol(data)-1]
#     gcontrol = list()
...
#     # devi = sum(devi_list)
#     maxlogpl_test = -(devi/2 + sum(log(W[Z & SUBSET])) + sum(Z & SUBSET))
#     # maxlogpl_test = -(devi/2 + sum(log(W[Z & SUBSET])) + sum(Z & SUBSET))
#     return(c(maxlogpl_test, sum(Z & SUBSET), devi))
#     # return(c(maxlogpl_test, length(fit_data$.mpl.Y), devi))
#   # }
# }
# 
...
#   # training_test_error = get_relative_error(training_maxlogpl / training_num, test_maxlogpl / test_num, 'training vs test')
#   # final_dataframe = rbind(training_validation_error, training_test_error)
#   # 
...
#   training_test1_error = get_relative_error(training_devi / training_num, test1_devi / test1_num, 'training vs test1')
#   training_test2_error = get_relative_error(training_devi / training_num, test2_devi / test1_num, 'training vs test2')
#   final_dataframe = rbind(training_validation_error, training_test1_error, training_test2_error)
#   
#   p = ggplot(data=final_dataframe, aes(x=shuffle_num, y=relative_error, group=type, color=type)) + geom_line()
#   p = p + xlab("Shuffle Number")
#   p = p + ylab("Percent Difference")
...
for (p in 1:length(P)){
  set.seed(shuf_num)
  P[[p]]$marks = sample(P[[p]]$marks, replace = T)
}
...
print(H)
Quad_all_all = mppm.quad.ppp(Y ~ marks, data=H, interaction = I, average_number = avg_particle_num, run_quad = F)
filename = file.path(data_dir, TMC, tissue, 'random_shuf', paste('quad_', n, '_', r, '_', hr, '_', intensity_type, '_across3_shuf_', shuf_num, '_self_dummy_grid_eps_20.Rda', sep = ''))
