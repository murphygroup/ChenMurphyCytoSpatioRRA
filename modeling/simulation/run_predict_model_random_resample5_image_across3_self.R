...
#       for (intensity_type in c('mean')) {
        ...
        hr = 1
            ...
            #if (!file.exists(file.path(data_dir, TMC, tissue, 'random_resample', paste('devi_', cluster_num, '_100-', radius, '_', hr, '_', intensity_type, '_', tissue, '_', tissue, '_across3_', img_idx, '_resample5_400_self_dummy_grid_eps_20.Rda', sep = '')))){
if(T){
print(paste('srun -p short1,interactive,pool1,pool3-bigmem,model1,model2,model3 -o ', file.path(data_dir, TMC, tissue, 'random_resample', paste('predict_', cluster_num, '_', radius, '_', hr, '_', intensity_type, '_across3_', img_idx, '_resample5_self.out', sep = '')), ' -n 1 -c 4 --mem 21G Rscript predict_model_random_resampled5_image_across3.R ', tissue, ' ', intensity_type, ' ', cluster_num, ' ', radius, ' ', hr, ' ', img_idx, ' &', sep = ''))
system(paste('srun -p pool1,model1,model2 -t 72:00:00 -o ', file.path(data_dir, TMC, tissue, 'random_resample', paste('predict_', cluster_num, '_', radius, '_', hr, '_', intensity_type, '_across3_', img_idx, '_resample5_self.out', sep = '')), ' -n 1 -c 4 --mem 21G Rscript predict_model_random_resample5_image_across3_self.R ', tissue, ' ', intensity_type, ' ', cluster_num, ' ', radius, ' ', hr, ' ', img_idx, ' &', sep = '')) 
               }
           } 
          }
    }
