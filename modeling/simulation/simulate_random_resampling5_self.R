...
intensity = cell_pattern$n / cell_pattern$window$xrange[2] / cell_pattern$window$yrange[2]
cell_pattern_random = rpoispp(intensity, win = cell_list[[1]]$window)
...
pattern_exist[, 3] = as.factor(sample(types, nrow(cell_pattern_random_df), replace=T, model_mark_prob))
cell_pattern_random_num = dim(cell_pattern_random_df)[1]
resample_num = ceiling(cell_pattern_random_num * resample_percent / 100)
print(resample_num)
if (resample_num != 0){
  for (i in 1:resample_num){
    point_current_idx = sample(1:cell_pattern_random_num, 1)
...
marked_simulated_pattern[[1]] = ppp(c(pattern_exist[,1]), c(pattern_exist[,2]), window = cell_pattern_random_window, marks = factor(pattern_exist[,3], levels = types))


filename1 = file.path(data_dir, TMC, tissue, 'random_resample', paste('random_simulated_pattern_', cluster_num, '_', radius, '_', hr, '_', intensity_type, '_across3_', img_idx, '_resample5_', resample_percent, '_self_dummy_grid_eps_20.Rda', sep = ''))
print(marked_simulated_pattern)
